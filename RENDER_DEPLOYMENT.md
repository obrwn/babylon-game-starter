...
└─────────────────────────────────────────────────────────────────────┘
         ...
         │  FROM node:22-alpine           │        │
         │  - npm ci                      │        │
         │  - npm run deploy:prepare && npm run build │
...
### 2. **Nginx Proxy Configuration** (`nginx.conf` + chat template)

[`nginx.conf`](nginx.conf) proxies multiplayer and API routes inline. **Chat** uses a separate include rendered at container start:

```nginx
# Chat Slayer — include generated by docker-entrypoint.sh from template + env
include /etc/nginx/snippets/chat-proxy.conf;
...
Chat proxy defaults live in [`deploy/chat-proxy.env.defaults`](deploy/chat-proxy.env.defaults) (generated by `npm run deploy:prepare`). [`docker-entrypoint.sh`](docker-entrypoint.sh) runs `envsubst` on [`src/deployment/templates/nginx.chat-proxy.conf.template`](src/deployment/templates/nginx.chat-proxy.conf.template) before nginx starts. Override upstream with **`CHAT_UPSTREAM_URL`** on Render. See [src/deployment/DEPLOYMENT.md — Chat proxy](src/deployment/DEPLOYMENT.md#chat-proxy-chat-slayer).
...
# Step 3: Build client (prepare generates chat proxy artifacts + host files)
npm run deploy:prepare && npm run build  # TypeScript + Babylon.js → /dist
...
The production `Dockerfile` copies a static **`multiplayer-server`** binary into the nginx image and starts it with **`docker-entrypoint.sh`**: the entrypoint renders the chat proxy config (`envsubst`), starts Go on **`:5000`** (nginx `proxy_pass` for multiplayer), then nginx on **10000**. Render sets `PORT=10000`; the entrypoint runs Go with **`PORT=5000`** so it does not inherit the nginx port.
...
- [ ] **Chat (optional)**: With `"serviceUrl": "/chat-api"` in `chat/config.json`, login/register works without browser CORS errors (nginx same-origin proxy). Optional: set `CHAT_UPSTREAM_URL` on Render to override default Chat Slayer upstream.
...
├── docker-entrypoint.sh                # Renders chat proxy; starts Go :5000 + nginx :10000
├── nginx.conf                          # Multiplayer/API proxy; includes runtime chat-proxy.conf
├── deploy/chat-proxy.env.defaults      # Chat upstream defaults (generated by deploy:prepare)
